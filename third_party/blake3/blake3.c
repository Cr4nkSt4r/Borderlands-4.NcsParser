...
INLINE void output_root_bytes(const output_t* self, uint64_t seek, uint8_t* out, size_t out_len) {
    ...
    while (out_len > 0) {
        blake3_compress_xof(
            self->input_cv, self->block, self->block_len, output_block_counter, self->flags | ROOT,
            wide_buf
        );
...
INLINE size_t compress_chunks_parallel(
    ...
    blake3_hash_many(
        chunks_array, chunks_array_len, BLAKE3_CHUNK_LEN / BLAKE3_BLOCK_LEN, key, chunk_counter,
...
INLINE size_t compress_parents_parallel(
    ...
    blake3_hash_many(
        parents_array, parents_array_len, 1, key,
    ...
    while (input_len > BLAKE3_CHUNK_LEN) {
        ...
        if (subtree_len <= BLAKE3_CHUNK_LEN) {
            ...
            compress_subtree_to_parent_node(
                input_bytes, subtree_len, self->key, self->chunk.chunk_counter, self->chunk.flags,
                cv_pair
            );

...
namespace bl4::ncs {
#if defined(_WIN32)
  void *load_library_handle(const fs::path &path) {
    const std::wstring path_w = path.c_str();
  ...
  void *get_export(void *handle, const char *name) {
    if (!handle) {
      return nullptr;
    }
    FARPROC address = ::GetProcAddress(reinterpret_cast<HMODULE>(handle), name);
  ...
  Fn require_export(void *handle, const char *name) {
    void *export_pointer = get_export(handle, name);
    if (!export_pointer) {
      throw std::runtime_error(std::string("Missing Oodle export: ") + name);
    }
    return reinterpret_cast<Fn>(export_pointer);
  }

  template <typename Fn>
  Fn optional_export(void *handle, const char *name) {
    void *export_pointer = get_export(handle, name);
    return reinterpret_cast<Fn>(export_pointer);
  }
  ...
  using OodleLZ_GetAllChunksCompressorFn = int (*)(const void *compBuf, long compBufSize, long rawLen);
  ...
  std::unique_ptr<OodleApi> OodleApi::try_load_default() {
    const fs::path oodle_lib_path = default_oodle_library_path();
    std::error_code error_code;
    if (!fs::exists(oodle_lib_path, error_code) || error_code) {
      return nullptr;
    }
    try {
      BL4_LOG_INFO("Using: %s", oodle_lib_path.filename().string().c_str());
      return OodleApi::load(oodle_lib_path);
    } catch (const std::exception &ex) {
      BL4_LOG_ERROR("Failed to load Oodle from %s: %s", oodle_lib_path.filename().string().c_str(), ex.what());
      return nullptr;
    }
  }

  std::unique_ptr<OodleApi> OodleApi::load(const fs::path &lib_path) {
    void *handle = load_library_handle(lib_path);
    if (!handle) {
      throw std::runtime_error(std::string("Failed to load Oodle library: ") + lib_path.string());
    }
    try {
      auto *decompress = require_export<void *>(handle, "OodleLZ_Decompress");
      auto *compress = require_export<void *>(handle, "OodleLZ_Compress");
      auto *get_size = require_export<void *>(handle, "OodleLZ_GetCompressedBufferSizeNeeded");
      auto *get_all_chunks = require_export<void *>(handle, "OodleLZ_GetAllChunksCompressor");

      void *get_default_opts = optional_export<void *>(handle, "OodleLZ_CompressOptions_GetDefault");
      void *get_cfg = optional_export<void *>(handle, "Oodle_GetConfigValues");
      void *set_cfg = optional_export<void *>(handle, "Oodle_SetConfigValues");

      return std::unique_ptr<OodleApi>(
          new OodleApi(handle, decompress, compress, get_size, get_default_opts, get_cfg, set_cfg, get_all_chunks));
    } catch (...) {
      unload_library_handle(handle);
      throw;
    }
  }
  ...
  void OodleApi::decompress(std::span<const std::uint8_t> compressed, std::span<std::uint8_t> decompressed) {
    if (compressed.empty()) {
    ...
    (
      ...
      decompressed.data(),
      static_cast<long>(decompressed.size()),
      1,
      1,
      0,
      nullptr,
  ...
  std::vector<std::uint8_t> OodleApi::compress(
    ...
    auto get_size = reinterpret_cast<OodleLZ_GetCompressedBufferSizeNeededFn>(get_comp_buf_size_fn_);
    ...
    if (max_out <= 0 || max_out > static_cast<long>(std::numeric_limits<int>::max())) {
      throw std::runtime_error(std::string("OodleLZ_GetCompressedBufferSizeNeeded returned invalid size"));
    }

    void *options = nullptr;
    if (get_default_options_fn_) {
      auto get_default = reinterpret_cast<OodleLZ_CompressOptions_GetDefaultFn>(get_default_options_fn_);
  ...
  OodleLZ_Compressor OodleApi::get_all_chunks_compressor(std::span<const std::uint8_t> compressed_chunk, std::size_t raw_len) {
    ...
    const int res = fn(compressed_chunk.data(), static_cast<long>(compressed_chunk.size()), static_cast<long>(raw_len));
